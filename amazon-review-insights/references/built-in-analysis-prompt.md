...
Analyze every unique collected review; never percentage-sample. The default collection cap is 2,000 per ASIN; display each member's actual `targetLimit` if customized. Display `Source-reported total: N` if a documented SellerSprite total was returned; display `Collected total: N reviews` if collection ended before the cap; otherwise display `Collected 2,000 reviews; source total unknown` (substitute the actual cap/count). Partial members always show the exact recorded failure and counts instead of a completion claim. Identify all three as SellerSprite service results, never as Amazon-wide verified totals. A custom user prompt always produces this HTML report.
